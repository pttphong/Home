...
namespace SQLServerDatabaseBackup
{
    public partial class frmMain : Form
    {
        ...
        private void frmMain_Load(object sender, EventArgs e)
        {
            ...
            string[] instances = CheckKeyRegedit();
        ...
        private string[] CheckKeyRegedit()
        {
            object _RegistryKey = null;
            try
            {
                // That works fine in Win32 but not in Win64
                string[] InstalledInstances_32bit = {};
                _RegistryKey = Registry.LocalMachine.OpenSubKey("SOFTWARE\\Microsoft\\Microsoft SQL Server").GetValue("InstalledInstances");
                if (_RegistryKey != null)
                {
                    InstalledInstances_32bit = (string[])_RegistryKey;
                }
                else
                {
                    _RegistryKey = Registry.LocalMachine.OpenSubKey("SOFTWARE\\Wow6432Node\\Microsoft\\Microsoft SQL Server").GetValue("InstalledInstances");
                    if (_RegistryKey != null)
                    {
                        InstalledInstances_32bit = (string[])_RegistryKey;
                    }
                }
                return InstalledInstances_32bit;
            }
            catch (Exception)
            {
                // That works fine in Win64 but not in Win32  
                string[] InstalledInstances_64bit = {};
                _RegistryKey = Registry.LocalMachine.OpenSubKey("\\SOFTWARE\\Microsoft\\Microsoft SQL Server").GetValue("InstalledInstances");
                if (_RegistryKey != null)
                {
                    InstalledInstances_64bit = (string[])_RegistryKey;
                }
                else
                {
                    _RegistryKey = Registry.LocalMachine.OpenSubKey("\\SOFTWARE\\Wow6432Node\\Microsoft\\Microsoft SQL Server").GetValue("InstalledInstances");
                ...
                return InstalledInstances_64bit;
            }
        }
